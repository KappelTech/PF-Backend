...
router.post("/find-user", findUserLimiter, async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            return res.status(200).json({
                message: 'User created, not Active',
...
router.post("/create-user", async (req, res, next) => {
    try {
        ...
        return res.status(200).json({ message: "User activated successfully.",  user, token: jwtToken, expiresIn: 3600 });
    } catch (error) {
...
router.post("/resend-code", async (req, res, next) => {
    try {
        ...
        if (token) {
            ...
            token.code = code;
            token.expiresAt = Date.now() + 10 * 60 * 1000; // 10 minutes from now
            await token.save();
        } else {
            // Create a new token
            token = new Token({
    ...
    try {
      const response = await axios.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [
          ...
          { role: 'user', content: `Generate a concise CrossFit-style ${duration} duration ${userPreferences} workout with the following equipment: ${selectedEquipment.join(', ')}. Include only the workout exercises, without any warm-up or cool-down instructions.` }
        ],
        max_tokens: 200,
        temperature: 0.7
      }, {
